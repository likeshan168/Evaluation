    ...
    public class DashboardController : Controller
    {
        ...
        private IEnumerable<DashboardPageModel> GetToDoList()
        {
            using (var conn = SqlConnections.NewByKey(Constants.Db.ConnectionName))
            {
                ...
                //else
                //{
                return conn.Query<DashboardPageModel>($"SELECT t.Title, t.Content, t.StartDate, t.EndDate, u2.Username as CreateBy, t.Url, t.ExamId FROM hr.ToDoList AS t LEFT JOIN dbo.Users AS u ON t.UserId = u.UserId LEFT JOIN dbo.Users AS u2 ON u2.UserId = t.CreateBy WHERE t.IsComplete = 0 AND t.EndDate >= CONVERT(VARCHAR(20),GETDATE(),23) and t.UserId={int.Parse(((UserDefinition)Authorization.UserDefinition).Id)}");
                //}
            }
        }
    }
